...
class SortingUtility {
    static func sortFiles(_ files: [URL], by option: SortOption, ascending: Bool) -> [URL] {
        switch option {
        case .name:
        ...
        case .tag:
            return files.sorted {
                let lhsTag = PlaylistManager.shared.fileTags[$0] ?? 0
                let rhsTag = PlaylistManager.shared.fileTags[$1] ?? 0
